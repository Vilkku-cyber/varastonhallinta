...
export default defineConfig(({ command, mode }) => {
  ...
  const storage = env.VITE_STORAGE || (env.VERCEL === '1' ? 'firebase' : 'local');
  ...
  if (command === 'build' && env.VERCEL === '1') {
    ...
    if (storage !== 'firebase' || required.some((key) => !env[key])) {
  ...
  return { define: { 'import.meta.env.VITE_STORAGE': JSON.stringify(storage), 'import.meta.env.VITE_DEMO_SETUP': JSON.stringify(env.VERCEL_ENV === 'preview' ? 'enabled' : 'disabled') } };
});
